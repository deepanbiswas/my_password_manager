...
[`backup.sh.template`](../infrastructure/templates/backup.sh.template) uploads to **`${RCLONE_REMOTE_NAME}:${RCLONE_BACKUP_FOLDER}/`** (defaults: remote **`gdrive`**, folder **`vaultwarden-backups-hetzner`**). Set **`RCLONE_REMOTE_NAME`** and **`RCLONE_BACKUP_FOLDER`** in `/opt/vaultwarden/.env` to match `rclone config` and the folder you create on Drive.
...
5. Set **`RCLONE_REMOTE_NAME=gdrive`** and **`RCLONE_BACKUP_FOLDER=vaultwarden-backups-hetzner`** (or your chosen folder name) in `/opt/vaultwarden/.env`.
6. One-time folder for [`backup.sh`](../infrastructure/templates/backup.sh.template):
   ```bash
   rclone mkdir "gdrive:vaultwarden-backups-hetzner" 2>/dev/null || true
   rclone lsd "gdrive:vaultwarden-backups-hetzner"
   ```
7. Test:
   ```bash
   cd /opt/vaultwarden && set -a && source .env && set +a && ./scripts/backup.sh
   ```
8. Confirm a **`.gpg`** file appears under **`vaultwarden-backups-hetzner`** (or your **`RCLONE_BACKUP_FOLDER`**) in Google Drive.
...
| Backup folder missing | Run `rclone mkdir "<remote>:vaultwarden-backups-hetzner"` once (or match **`RCLONE_BACKUP_FOLDER`**) |
...
1. **Rclone** — OAuth remote configured; name = **`RCLONE_REMOTE_NAME`**; remote folder **`${RCLONE_BACKUP_FOLDER:-vaultwarden-backups-hetzner}/`** exists.
